...
def check_keydown_events(event, ai_settings, screen, ship, bullets):
    ...
    if event.key == pygame.K_q:
        sys.exit()
    elif event.key == pygame.K_RIGHT:     #Start moving the ship right
        ship.moving_right = True
    elif event.key == pygame.K_LEFT:    #Start moving the ship left
        ship.moving_left = True
    elif event.key == pygame.K_SPACE:   #Fire the bullet and add it to the group
        fire_bullet(ai_settings, screen, ship, bullets)

def check_keyup_events(event, ship):
    """Respond to key releases."""
    
    if event.key == pygame.K_RIGHT:     #Stop moving the ship right
        ship.moving_right = False
    elif event.key == pygame.K_LEFT:    #Stop moving the ship left
...
def check_bullet_alien_collision(ai_settings, screen, ship, bullets, aliens):
    ...
    #Get rid of the bullet and alien that have collided
...
def check_aliens_bottom(ai_settings, stats, screen, ship, aliens, bullets):
    ...
    for alien in aliens.sprites():
        if alien.rect.bottom >= screen_rect.bottom:
            #Treat this the sam as if the ship got hit
